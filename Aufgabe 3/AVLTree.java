public class AVLTree<T> {
    ...
    private class Fork extends Node {
        ...
        private Fork singleRotateLeft(){
          Fork temp = (Fork)this.right;
          this.setRight(temp.left);
          temp.setLeft(this);
          return temp;
        }

        private Fork singleRotateRight(){
          Fork temp = (Fork)this.left;
          this.setLeft(temp.right);
          temp.setRight(this);
          return temp;
        }
        ...
        private Node repairAVL(){
          if(getBalanceNumber() > 1){
            if(this.right.getBalanceNumber() >= 0)
              return this.singleRotateLeft();
            else
              return this.doubleRotateRightLeft();
          }else if(getBalanceNumber() < -1){
            if(this.left.getBalanceNumber() < 0)
              return this.singleRotateRight();
            else
              return this.doubleRotateLeftRight();
          }else
            return this;
        }
        ...
        public Node delete(int key){
          if(key <= this.key){
            this.setLeft(this.left.delete(key));
            if(this.left == null)
              return right;
          }else{
